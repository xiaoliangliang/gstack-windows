...
| Login session | `login-session headed [url]`, `login-session headless`, `login-session status`, `login-session stop` | Reuse a gstack-managed persistent Chrome login on Windows |
...
1. **First call**: CLI checks `.gstack/browse.json` (in the project root) for a running server. None found — it spawns `bun run browse/src/server.ts` in the background. The server launches Chromium via Playwright, picks a random port (10000-60000), generates a bearer token, writes the state file, and starts accepting HTTP requests. By default this is headless and ephemeral, but `login-session headed/headless` can switch it into a persistent Chrome profile. Startup takes ~3 seconds.
...
### Windows authenticated browsing

On Windows, direct cookie import from a real browser is not always possible. Modern Chrome and Edge default profiles may use app-bound cookie encryption, which blocks out-of-process decryption by tools like gstack. When that happens, the supported workflow is:

1. `browse login-session headed <url>`
2. Sign in manually in the gstack-managed Chrome window
3. `browse login-session headless`
4. Reuse that persistent logged-in profile from normal `browse` commands

This is the recommended path for sites like Douyin where keeping a legitimate login session matters more than raw cookie extraction.
...
| `browse/src/cookie-import-browser.ts` | Imports Chromium cookies when the source profile allows decryption, powers the interactive picker UI, and reports Windows app-bound limitations with login-session fallback guidance. |
...
The active Codex install usually lives at `~/.codex/skills/gstack/`. After making changes:
