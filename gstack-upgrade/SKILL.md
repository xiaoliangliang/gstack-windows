...
do
  if [ -x "$_candidate/bin/gstack-config" ]; then
    GSTACK_DIR="$_candidate"
    break
  fi
done
_AUTO=""
[ "${GSTACK_AUTO_UPGRADE:-}" = "1" ] && _AUTO="true"
[ -z "$_AUTO" ] && _AUTO=$([ -n "$GSTACK_DIR" ] && "$GSTACK_DIR/bin/gstack-config" get auto_upgrade 2>/dev/null || true)
...
```bash
"$GSTACK_DIR/bin/gstack-config" set auto_upgrade true
```
...
```
Tell user: "Update checks disabled. Run `\"$GSTACK_DIR/bin/gstack-config\" set update_check true` to re-enable."
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/xiaoliangliang/gstack-windows.git "$TMP_DIR/gstack"
...
LOCAL_GSTACK=""
for _local_candidate in "$_ROOT/.codex/skills/gstack" "$_ROOT/.claude/skills/gstack"; do
  if [ -n "$_ROOT" ] && [ -d "$_local_candidate" ]; then
    _RESOLVED_LOCAL=$(cd "$_local_candidate" && pwd -P)
    _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
    if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
      LOCAL_GSTACK="$_local_candidate"
      break
    fi
  fi
done
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit that vendored gstack directory when you're ready."
...
```bash
"$GSTACK_DIR/bin/gstack-update-check" --force
```
