...
import { defaultTempPath, getSafeDirectories, validateSafePath } from './path-utils';
...
export async function handleSnapshot(
  ...
  if (opts.annotate) {
    const screenshotPath = opts.outputPath || defaultTempPath('browse-annotated.png');
    // Validate output path (consistent with screenshot/pdf/responsive)
    validateSafePath(screenshotPath, getSafeDirectories());
    try {
