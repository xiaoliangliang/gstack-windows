...
import { getSafeDirectories, validateSafePath } from './path-utils';
  ...
  switch (command) {
    ...
    case 'cookie-import': {
      ...
      if (path.isAbsolute(filePath)) {
        validateSafePath(filePath, getSafeDirectories());
      }
    ...
    case 'cookie-import-browser': {
      const { findInstalledBrowsers, importCookies } = await import('./cookie-import-browser');
      // Two modes:
      ...
      if (domainIdx !== -1 && domainIdx + 1 < args.length) {
        ...
        const browser = browserArg || (process.platform === 'win32' ? 'chrome' : 'comet');
      ...
      try {
        await openExternalUrl(pickerUrl);
      } catch {
