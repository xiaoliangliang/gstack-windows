...
import { defaultTempPath, validateSafePath } from './path-utils';
...
export async function handleMetaCommand(
  ...
  switch (command) {
    ...
    case 'status': {
      ...
      const launch = bm.getLaunchInfo();
      return [
        `Status: healthy`,
        `URL: ${page.url()}`,
        `Tabs: ${tabs}`,
        `Browser: ${launch.browserName}${launch.headless ? ' (headless)' : ' (headed)'}`,
        `Session: ${launch.mode}${launch.profileDir ? ` → ${launch.profileDir}` : ''}`,
        `PID: ${process.pid}`,
      ].join('\n');
    }
    ...
    case 'screenshot': {
      ...
      const page = bm.getPage();
      let outputPath = defaultTempPath('browse-screenshot.png');
    ...
    case 'pdf': {
      const page = bm.getPage();
      const pdfPath = args[0] || defaultTempPath('browse-page.pdf');
    ...
    case 'responsive': {
      const page = bm.getPage();
      const prefix = args[0] || defaultTempPath('browse-responsive');
      validateSafePath(prefix);
      const viewports = [
